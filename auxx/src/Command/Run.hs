...
Avaliable commands:
   ...
   propose-update <N> <block ver> <script ver> <slot duration> <max block size> <software ver> <propose_file>? [vote-all]
...
runCmd sendActions (ProposeUpdate idx params voteAll) =
    Update.propose sendActions idx params voteAll
